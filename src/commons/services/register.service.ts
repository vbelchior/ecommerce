...
export class RegisterService {
	...
	public update(id: string, entity: RegisterModel, cascade?: boolean): Observable<any> {
		const path: string = `${environment.server}/registers/${id}`;
		let query: HttpParams = new HttpParams();
		if (TypeUtil.exists(cascade)) query = query.set('cascade', String(cascade));
	...
	public count(personId?: number, date?: Date, dateEnd?: Date, status?: number, risk?: number, priority?: number, isOnline?: boolean): Observable<number> {
		let query: HttpParams = new HttpParams();
